...
async function fetchAlertAnalytics() {
	...
	for (const proxy of [
		...
		try { const r=await fetch(proxy,{cache:'no-store'}); if(r.ok){data=await r.json();break;} } catch(e){}
	}
	...
	_analyticsRawEvents=data; _analyticsFilter='all'; _analyticsChartRange='24h';
	renderAnalytics();
}
...
function buildAnalyticsHTML(events) {
	...
	// ── Point 10: Iron Dome battery coverage ──────────────────────────────
	const IRON_DOME_BATTERIES = [
		{ name:'Kiryat Shmona',   region:'North',       src:'Hezbollah (Lebanon)', reported:'Reuters, 2024', c:'#fb923c' },
		{ name:'Haifa',           region:'North',       src:'Hezbollah (Lebanon)', reported:'Times of Israel, 2023', c:'#fb923c' },
		{ name:'Tel Aviv (×3)',   region:'Center',      src:'Long-range missiles',  reported:'IDF, 2023', c:'#60a5fa' },
		{ name:'Ashkelon',        region:'South (Gaza)',src:'Hamas / PIJ',          reported:'AP, 2021', c:'#f87171' },
		{ name:'Beersheba',       region:'South (Gaza)',src:'Hamas / PIJ',          reported:'IDF, 2021', c:'#f87171' },
		{ name:'Jerusalem',       region:'Jerusalem',   src:'Multiple',             reported:'Haaretz, 2023', c:'#a78bfa' },
		{ name:'Eilat',           region:'South',       src:'Houthis (Yemen)',      reported:'IDF, 2024', c:'#f87171' },
		{ name:'Golan Heights',   region:'North',       src:'Hezbollah / Syria',    reported:'Times of Israel, 2024', c:'#fb923c' },
	];
	const coveredRegions = new Set(IRON_DOME_BATTERIES.map(b=>b.region));
	const alertRegions   = new Set(Object.keys(regHits));
	const uncovered = [...alertRegions].filter(r=>![...coveredRegions].some(cr=>cr.includes(r)||r.includes(cr)));
	html += card(`
		${lbl('Iron Dome Battery Coverage (Publicly Reported)')}
		<div style="font-size:0.7rem;color:rgba(255,255,255,0.28);margin-bottom:0.7rem;">Sources: IDF statements, Reuters, AP, Times of Israel. Locations are approximate — batteries are mobile. Does not reflect classified deployments.</div>
		<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:0.45rem;margin-bottom:0.75rem;">
			${IRON_DOME_BATTERIES.map(b=>`
				<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-left:3px solid ${b.c};border-radius:6px;padding:0.5rem 0.65rem;">
					<div style="color:white;font-weight:700;font-size:0.82rem;">🛡 ${b.name}</div>
					<div style="font-size:0.7rem;color:${b.c};">${b.region}</div>
					<div style="font-size:0.65rem;color:rgba(255,255,255,0.28);margin-top:0.1rem;">Threat: ${b.src}</div>
					<div style="font-size:0.6rem;color:rgba(255,255,255,0.18);">Source: ${b.reported}</div>
				</div>
			`).join('')}
		</div>
		<div style="display:flex;gap:0.75rem;flex-wrap:wrap;align-items:flex-start;">
			<div style="flex:1;min-width:150px;">
				<div style="font-size:0.72rem;color:#34d399;font-weight:700;margin-bottom:0.3rem;">✓ Regions with reported coverage:</div>
				${[...coveredRegions].map(r=>`<div style="font-size:0.75rem;color:rgba(255,255,255,0.55);margin-bottom:0.15rem;">• ${r}</div>`).join('')}
			</div>
			${uncovered.length?`<div style="flex:1;min-width:150px;">
				<div style="font-size:0.72rem;color:#fbbf24;font-weight:700;margin-bottom:0.3rem;">⚠ Alert regions with no known battery:</div>
				${uncovered.map(r=>`<div style="font-size:0.75rem;color:rgba(255,255,255,0.55);margin-bottom:0.15rem;">• ${r}</div>`).join('')}
			</div>`:''}
		</div>
	`);
...
async function fetchTelegramIntercepts() {
	...
	if (!html) {
		div.innerHTML = '<span style="color:rgba(255,255,255,0.25);font-size:0.8rem;">Could not reach Telegram. Try disabling ad blockers or VPN.</span>';
		return;
	}
	// Extract message texts from Telegram channel HTML
	const parser   = new DOMParser();
	const doc      = parser.parseFromString(html, 'text/html');
	const msgEls   = doc.querySelectorAll('.tgme_widget_message_text');
	// Intercept-related keywords
	...
	if (!found.length) {
		div.innerHTML = `<div style="color:rgba(255,255,255,0.25);font-size:0.8rem;">No recent intercept reports found in last Telegram messages. <a href="https://t.me/IDFSpokesperson" target="_blank" style="color:#34d399;">Open channel ↗</a></div>`;
		return;
	}
	div.innerHTML = `
		<div style="display:flex;flex-direction:column;gap:0.55rem;width:100%;">
			...
			`).join('')}
			<a href="https://t.me/IDFSpokesperson" target="_blank" style="font-size:0.72rem;color:rgba(52,211,153,0.6);text-align:right;">Open full IDF Spokesperson channel ↗</a>
		</div>`;
}
